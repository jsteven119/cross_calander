...
// plans 탭의 열 순서 (A=id ... R=updatedBy)
const COLUMNS = [
  'id', 'region', 'market', 'team', 'owner', 'type', 'product', 'hero',
...
export async function fetchGTMData(): Promise<GTMData> {
  ...
  const res = await sheets.spreadsheets.values.get({
    ...
    range: `${sheetName}!A2:R1000`,   // 1행은 헤더
    valueRenderOption: 'FORMATTED_VALUE',
  })

  const rows = (res.data.values ?? []) as unknown[][]
  const activities = rows
    .filter(r => txt(r[0]) || txt(r[8]))   // id 또는 title 있는 행만
    .map(rowToActivity)
...
export function getSampleData(): GTMData {
  const raw: Array<Partial<GTMActivity>> = [
    { id: 'P-0001', region: '국내', market: '올리브영', team: '국내마케팅', owner: '김지은', type: '신제품출시', product: '헬로키티 컬렉션', hero: true, title: '헬로키티 립/아이 컬렉션 런칭', startDate: '2026-04-10', endDate: '2026-05-30', status: '진행중', budget: '1.5억', channel: '올리브영', issue: '초도 물량 부족 우려', riskLevel: '중', updatedBy: '김지은' },
    { id: 'P-0002', region: '미주', market: '미국-아마존', team: '미주팀', owner: 'Sarah K', type: '신제품출시', product: '헬로키티 컬렉션', hero: true, title: 'Hello Kitty Collection US Launch', startDate: '2026-04-20', endDate: '2026-06-15', status: '확정', budget: '$120K', channel: 'Amazon', issue: '통관 지연 가능성', riskLevel: '상', updatedBy: 'Sarah K' },
