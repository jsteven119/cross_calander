...
여러 팀·여러 권역(국내/미주/중화권/일본)·여러 브랜드(웨이크메이크/컬러그램/바이오힐보)의
GTM 활동을 **실시간으로 공유**하는 통합 캘린더.
서로 어떤 상품을 언제 진행하는지, 어디서 충돌·이슈가 나는지 한눈에 봅니다.

## 무엇을 해결하나

- **서로 뭘 하는지 모름** → 전 권역 스윔레인 타임라인 한 화면
- **주력 상품 변동 심함** → 상품 렌즈로 권역 넘나들며 추적 + 변경 피드
- **권역별 EC 프로모션 산재** → 권역 × EC채널 프로모션 보드로 정리
- **브랜드별 추적 필요** → 브랜드 필터(웨이크메이크/컬러그램/바이오힐보)
